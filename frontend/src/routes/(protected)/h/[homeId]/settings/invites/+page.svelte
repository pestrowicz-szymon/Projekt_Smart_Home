...
<svelte:head><title>Access Management</title></svelte:head>
						...
						<div class="min-w-0 flex-1">
							<div class="flex items-center gap-2">
								<code class="break-all rounded bg-surface-sunken px-2 py-1 font-mono text-xs">
									{invite.code_hash ? invite.code_hash.substring(0, 16) + '...' : 'Hidden'}
								</code>
							...
							<p class="mt-1 text-xs text-foreground-muted">
								{#if invite.used_at}
									Used by {invite.used_by?.first_name}
									{invite.used_by?.last_name} on {formatDate(invite.used_at)}
