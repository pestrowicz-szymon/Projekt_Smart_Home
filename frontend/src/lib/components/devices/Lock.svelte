...
<div class="flex items-center justify-between">
	<span class="text-sm">State: {device.current_state > 0 ? 'Unlocked' : 'Locked'}</span>
	<form
		method="POST"
		action="?/control"
		use:enhance={({ formData }) => {
			...
			return async ({ result }) => {
				...
				if (result.type === 'failure') {
					...
					const targetState = actionType === 'unlock' ? 1 : 0;
	...
	>
		...
		<input type="hidden" name="actionType" value={device.current_state > 0 ? 'lock' : 'unlock'} />
		<button
			type="submit"
			disabled={loading}
		...
		>
			...
			<span class="transition-opacity duration-150 {loading ? 'opacity-0' : ''}">
				{device.current_state > 0 ? 'Lock' : 'Unlock'}
			</span>
		</button>
	</form>
</div>
