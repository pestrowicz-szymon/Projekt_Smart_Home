...
from .services import build_command_ack_topic, build_command_topic, build_telemetry_topic, mark_action_acked, record_sensor_reading
...
def _configure_security(client: mqtt.Client, settings: MQTTConnectionSettings) -> None:
	...
	if settings.ca_certs:
		tls_kwargs['ca_certs'] = settings.ca_certs
	if settings.client_cert:
		tls_kwargs['certfile'] = settings.client_cert
	if settings.client_key:
		tls_kwargs['keyfile'] = settings.client_key
...
def _handle_telemetry(topic: str, payload: dict[str, Any]) -> None:
	...
	device = Device.objects.select_related('home').filter(home_id=home_id, hardware_id=hardware_id).first()
	if device is None:
...
def _handle_command_ack(topic: str, payload: dict[str, Any]) -> None:
	...
	action = (
		...
		.filter(device__home_id=home_id, device__hardware_id=hardware_id, correlation_id=correlation_id)
		.first()
	)
	if action is None:
...
def publish_device_action(action: DeviceAction) -> None:
	...
	payload = {
		...
		'home_id': action.device.home_id,
		'device_id': action.device.id,
		'hardware_id': action.device.hardware_id,
		'action_type': action.action_type,
		'payload': action.payload,
	}
	info = client.publish(build_command_topic(action.device.home_id, action.device.hardware_id), json.dumps(payload), qos=1)
	info.wait_for_publish()
	client.loop_stop()
	client.disconnect()
	logger.info('Published MQTT action %s for device %s', action.correlation_id, action.device.hardware_id)
