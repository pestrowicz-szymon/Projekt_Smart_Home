...
class Home(models.Model):
    ...
    owner = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="owned_homes"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["home", "name"], name="unique_room_per_home"
            ),
        ]
        indexes = [
...
class Gateway(models.Model):
    ...
    home = models.ForeignKey(
        Home, on_delete=models.SET_NULL, related_name="gateways", blank=True, null=True
    )
    name = models.CharField(max_length=100, blank=True, default="")
    hardware_id = models.CharField(max_length=100, unique=True)  # CN from certificate
    ...
    pairing_code = models.CharField(max_length=6, blank=True, null=True)
    ...
    def __str__(self):
        return (
            f"Gateway {self.hardware_id} ({self.home if self.home else 'Unassigned'})"
        )
...
class Device(models.Model):
    ...
    home = models.ForeignKey(
        Home, on_delete=models.CASCADE, related_name="devices", blank=True, null=True
    )
    ...
    room = models.ForeignKey(
        Room, on_delete=models.SET_NULL, related_name="devices", blank=True, null=True
    )
    name = models.CharField(max_length=100)
