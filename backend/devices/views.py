...
class DeviceEventStreamView(APIView):
    """
    Server-Sent Events (SSE) endpoint for real-time device updates.
    Subscribes to PostgreSQL NOTIFY channels for all homes the user has access to.
    Fully asynchronous implementation optimized for ASGI/DRF.
    """
    ...
    @extend_schema(
        tags=["devices"],
        summary="Stream real-time device events",
        description="Connect to this endpoint using EventSource to receive real-time updates for devices in your homes.",
        responses={200: OpenApiTypes.STR},
    )
    def get(self, request):
        user = request.user

        # Get homes the user has access to (Synchronous call is safe here)
        home_ids = list(
            Home.objects.filter(Q(owner=user) | Q(memberships__user=user))
        ...
        if not home_ids:
            ...
            async def empty_generator():
                yield ": no homes found\n\n"
                while True:
                    await asyncio.sleep(30)
                    yield ": keep-alive\n\n"

            return StreamingHttpResponse(
                empty_generator(), content_type="text/event-stream"
            )
        ...
        async def event_generator():
            ...
            logger.info(f"SSE: Connecting (Async) to database for user {user.username}")
...
class HomeViewSet(viewsets.ModelViewSet):
    ...
    def members(self, request, pk=None):
        ...
        if not user_has_home_access(request.user, home, write=True):
            return Response(
                {
                    "detail": "You do not have permission to manage members for this home."
                },
    ...
    def member(self, request, pk=None, member_id=None):
        ...
        if member is None:
            return Response(
                {"detail": "Home member not found."}, status=status.HTTP_404_NOT_FOUND
            )
...
@extend_schema_view(
    list=extend_schema(
        tags=["rooms"],
        summary="List rooms",
        parameters=[HOME_ID_QUERY_PARAMETER],
        examples=[ROOM_RESPONSE_EXAMPLE],
    ...
    create=extend_schema(
        ...
        examples=[ROOM_CREATE_EXAMPLE, ROOM_RESPONSE_EXAMPLE],
        responses=RoomSerializer,
    ),
    update=extend_schema(
        tags=["rooms"],
...
class RoomViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        queryset = Room.objects.select_related("home", "home__owner").prefetch_related(
            "devices"
        )
        ...
        return queryset.filter(
            Q(home__owner=user) | Q(home__memberships__user=user)
        ).distinct()
...
@extend_schema_view(
    ...
    retrieve=extend_schema(
        ...
        examples=[DEVICE_RESPONSE_EXAMPLE],
    ...
    create=extend_schema(
        ...
        examples=[DEVICE_CREATE_EXAMPLE, DEVICE_RESPONSE_EXAMPLE],
        responses=DeviceSerializer,
    ),
    update=extend_schema(
        tags=["devices"],
    ...
    destroy=extend_schema(
        tags=["devices"], summary="Delete device", responses={204: None}
    ),
)
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not permission.has_object_permission(request, self, device):
            return Response(
                {"detail": "Only the home owner can delete this device."},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def readings(self, request, pk=None):
        ...
        return Response(
            SensorDataSerializer(reading).data, status=status.HTTP_201_CREATED
        )
    ...
    def latest_reading(self, request, pk=None):
        ...
        if reading is None:
            return Response(
                {"detail": "No readings found."}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def actions(self, request, pk=None):
        ...
        return Response(
            DeviceActionSerializer(action).data, status=status.HTTP_201_CREATED
        )
...
@extend_schema_view(
    list=extend_schema(
        ...
        responses=DeviceActionSerializer,
    ),
    retrieve=extend_schema(
        tags=["device-actions"],
...
class DeviceActionViewSet(
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            return queryset
        return queryset.filter(
            Q(device__home__owner=user) | Q(device__home__memberships__user=user)
        ).distinct()
